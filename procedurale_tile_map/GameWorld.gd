...
func setup() -> void:
	...
	var map_size_pixel := size * _tile_map.cell_size
	var mode = SceneTree.STRETCH_MODE_2D
	var aspect = SceneTree.STRETCH_ASPECT_KEEP
	get_tree().set_screen_stretch(mode, aspect, map_size_pixel)
	OS.set_window_size(2 * map_size_pixel)
...
func get_random_tile(probability:float) -> int:
	var is_ground = _rng.randf() < probability
	return (_pick_random_texture(Cell.GROUND) if is_ground 
		else _pick_random_texture(Cell.OBSTACLE))
